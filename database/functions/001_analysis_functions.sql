...
as $$
  select *
  from public.fn_get_train_schedule((
    select t.id
...
as $$
  select
    ...
    tr.status,
    (
      tr.status = 'ACTIVE'::public.track_status
...
create or replace function public.fn_find_nearby_stations(
...
as $$
  ...
  where s.status = 'ACTIVE'::public.station_status
...
create or replace function public.fn_estimate_delay_minutes(
  ...
  p_severity public.disruption_severity
)
returns integer
language sql
immutable
as $$
  select greatest(
    ...
    ceil(
      ...
      case p_severity
        when 'LOW'::public.disruption_severity then 1.10
        when 'MEDIUM'::public.disruption_severity then 1.25
        when 'HIGH'::public.disruption_severity then 1.50
        when 'CRITICAL'::public.disruption_severity then 2.00
        else 1.00
      end
    )::integer
  );
...
create or replace function public.fn_get_disruption_context(p_disruption_id uuid)
returns table (
  disruption_id uuid,
...
create or replace function public.fn_find_trains_using_track(
  p_track_id uuid,
...
as $$
  ...
  schedule_edges as (
    ...
    from public.trains t
    join public.train_schedules from_stop on from_stop.train_id = t.id
    join public.train_schedules to_stop
  ...
  join public.trains t on t.id = se.train_id
  join public.train_journeys tj
    on tj.train_id = t.id
